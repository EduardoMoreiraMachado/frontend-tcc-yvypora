...
export const MyOrderPage = () => {
  ...
  useEffect(() => {
    if (finish) {
      notifyAsForm('Entrega Finilizada, Recebeu seus pedidos ?', async () => {
            ...
            <LoadScript googleMapsApiKey='AIzaSyCQfb1qbVCwR8biTe469v_uBuVWaO9JxtY'>
              <div className={styles['route-container']}>
                <Route
                  ...
                  destination={order?.routes?.arrived ? order.routes.arrived : [0,0]}
                  origin={order?.routes?.origin? order.routes.origin : [0,0]}
                  waypoints={order?.routes?.waypoints ? order.routes.waypoints : [[0,0]]}
                />
              </div>
              {deliveryman ? (
                <>
                  ...
                  <div className={styles['delivery-man-card']}>
                    ...
                    <div className={styles['delivery-info']}>
                      <div className={styles['info-text']}>
                        <h2>Distância: </h2>
                        <span>{distance.toFixed(2).toString().replace(/\./g, ",")} km</span>
                      </div>
                      <div className={styles['info-text']}>
                        <h2>Tempo estimado: </h2>
                        <span>{time.toFixed(2).toString().replace(/\./g, ",")} minutos</span>
                      </div>
                    </div>
