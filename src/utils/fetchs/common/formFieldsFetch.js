...
export const fetchFairFormFields = async ({ latitude, longitude }) => {
  const { data } = await commonsAPI.get(`forms/fairs?latitude=${latitude}&longitude=${longitude}`);
...
export const fetchMarketerFormFields = async ({latitude, longitude}) => {
  const { data } = await commonsAPI.get(`forms/marketer?latitude=${latitude}&longitude=${longitude}`);
